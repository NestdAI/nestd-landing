...
import { phones, icon } from "./storefront.mjs";
...
function home(t, lang) {
  ...
  return `<section class="hero" data-section="hero"><div class="hero-copy"><p class="eyebrow hero-badge">${icon("home")}${s.badge}</p><h1>${s.hero}</h1><p class="hero-intro">${s.intro}</p><div class="hero-actions">${cta(t, "hero")}<a class="text-link" href="#how-it-works">${conversion[lang].demoLink} <span aria-hidden="true">↓</span></a></div><p class="paid-note">${t.paid}</p></div>${phones(t, lang)}</section>${searchNarrative(lang)}${alertNarrative(lang)}${demo(lang)}${clarity(lang)}<section class="section pricing-section" id="pricing" data-section="pricing"><div><p class="eyebrow">${t.pricingLabel}</p><h2>${t.pricingTitle}</h2><p class="section-intro">${t.pricingIntro}</p><p class="fine-print">${t.noGuarantee}</p><a class="text-link" href="${pathFor(lang, "pricing")}">${t.priceLink} ${arrow}</a></div>${priceCard(t, "home_pricing")}</section>${faq(t, lang)}${download(t)}`;
}
